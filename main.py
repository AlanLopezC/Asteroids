"""
This file is a copy of the code in CodeSkulptor.

! You need to run it in CodeSkulptor web app for it to work.
"""
...
def draw(canvas):
    global time, started, lives, rock_group, score
    ...
    for missile in set(missile_group):
        missile.draw(canvas)    
        finit = missile.update() # Age > lifespan
        if finit:
            missile_group.remove(missile)
    
    # Rock-missile collide
...
def rock_spawner():
    ...
    if started:
        ...
        if len(rock_group) < 12 and dist(new_rock.pos, my_ship.pos) > my_ship.radius:
...
def group_group_collide(group1, group2):
    ...
    for object1 in set(group1):
        for object2 in set(group2):
            boom = object1.collide(object2)
            if boom:
                group1.discard(object1)
                group2.discard(object2)
                score += 1
